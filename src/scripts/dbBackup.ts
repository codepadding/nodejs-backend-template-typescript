...
import { deleteFileFromGC, uploadFileToGC } from '../utils/googleCloud';
import fs from 'fs';
...
function convertToDate(dateString: string): Date | null {
  ...
  if (dateParts.length === 3 && dateParts.every(part => !isNaN(part))) {
    ...
    console.error('Invalid date format. Please provide date in dd_mm_yyyy format.');
    return null;
  }
}
...
export default async function createDbBackup(): Promise<void> {
  ...
  await exec(command, async (error: ExecException | null) => {
    ...
    uploadFileToGC(`db_backups/${formattedDate}.zip`, `db_backups/${formattedDate}.zip`)
